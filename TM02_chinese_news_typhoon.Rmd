...
```{r}
segment_not <- c("第卅六條", "第卅八條", "蘇南成", "災前", "災後", "莫拉克", "颱風", "應變中心", "停班停課", "停課", "停班", "停駛", "路樹", "里長", "賀伯", "採收", "菜價", "蘇迪", "受災戶", "颱風警報", "韋恩", "台東縣", "馬總統", "豪大雨", "梅姬", "台東", "台北市政府", "工務段", "漂流木", "陳菊", "台南縣", "卡玫基", "魚塭", "救助金", "陳情", "全省", "強颱", "中颱", "輕颱", "小林村", "野溪", "蚵民", "農委會", "來襲", "中油公司", "蔣總統經國", "颱風天", "土石流", "蘇迪勒", "水利署", "陳說", "颱風假", "颱風地區", "台灣", "臺灣", "柯羅莎", "八八水災", "紓困","傅崑萁", "傅崐萁","台中", "文旦柚", "鄉鎮市公所", "鄉鎮市", "房屋稅", "高雄", "未達", "台灣省", "台北市")
cutter <- worker()
...
```{r}
# recombined.df <- tokenized.df %>%
#     select(doc_id, word) %>%
#     unnest(word) %>%
#     group_by(doc_id) %>%
#     summarize(string = paste0(word, collapse = ",")) %>%
#     ungroup()
# recombined.df %>% head() %>% View()
...
five.gram <- unnested.df %>%
    ...
    mutate(w4 = lead(word, 3)) %>%
    mutate(w5 = lead(word, 4)) %>%
    ungroup() %>%
    filter(complete.cases(.)) %>%
    mutate(w12 = paste0(word, " ", w2)) %>%
    mutate(w13 = paste0(word, " ", w3)) %>%
    mutate(w14 = paste0(word, " ", w4)) %>%
    mutate(w15 = paste0(word, " ", w5))
...
bigrams %>% slice(1:20) %>% View()

bigrams.count <- bigrams %>%
    count(word1, word2, sort = T)

bigrams.count %>% slice(200:400) %>% View()
